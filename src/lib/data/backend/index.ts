...
export const backendChallenges: Challenge[] = [
  ...
  },
  {
    ...
    walkthrough: [
      {
        title: 'Entity and DTO Type Definitions',
        lines: [1, 22],
        what: 'Defines the User entity (what the database returns), filter criteria, and Data Transfer Objects for creating and updating users. UpdateUserDTO uses Partial to make all fields optional.',
        why: 'Separating the entity from DTOs enforces clear boundaries: the entity represents the full database record, while DTOs represent what the API accepts. Partial<CreateUserDTO> avoids duplicating field definitions for updates.',
        when: 'Define separate DTOs whenever your API input shape differs from your database entity. This is standard in layered architectures and prevents leaking internal structure to API consumers.',
        how: 'TypeScript interfaces define the shapes at compile time. The Partial utility type makes all properties of CreateUserDTO optional, which is exactly what you want for PATCH-style updates.',
      },
      {
        title: 'Repository Interface (Port)',
        lines: [24, 31],
        what: 'Defines the IUserRepository interface — a contract specifying all data access operations without any implementation details. This is the "port" in ports-and-adapters architecture.',
        why: 'The interface is the foundation of dependency inversion. The service layer depends on this abstraction, not on any database. You can swap PostgreSQL for MongoDB, or use an in-memory implementation for tests, without changing any business logic.',
        when: 'Define repository interfaces whenever you want to decouple business logic from data storage. This is essential for testability (mock repositories in unit tests) and flexibility (migrate databases without rewriting services).',
        how: 'All methods return Promises because data access is inherently asynchronous. The interface specifies input/output types but says nothing about SQL, HTTP, or any concrete technology.',
      },
      {
        title: 'Concrete Implementation - Constructor and Simple Queries',
        lines: [33, 40],
        what: 'The PostgresUserRepository class implements the interface with actual PostgreSQL queries. The constructor receives a database connection via dependency injection. findById executes a parameterized query.',
        why: 'Constructor injection makes the dependency explicit and testable. Parameterized queries ($1 syntax) prevent SQL injection — never concatenate user input into SQL strings.',
        when: 'Create a concrete repository class for each storage backend you need. In production, you might have PostgresUserRepository; in tests, you might have InMemoryUserRepository — both implementing the same interface.',
        how: 'The "implements IUserRepository" clause enforces that this class provides all methods the interface defines. TypeScript will error if any method is missing or has the wrong signature.',
      },
      {
        title: 'Complex Query Building and Mutations',
        lines: [42, 77],
        what: 'Implements findAll with dynamic query building (filters, pagination), create with INSERT RETURNING, update with dynamic SET clauses, and delete with row count checking.',
        why: 'Dynamic query building handles optional filters without requiring separate queries for each combination. RETURNING * gets the modified row back in one round trip. The update method dynamically builds SET clauses only for provided fields.',
        when: 'Use dynamic query building when filter combinations vary. Use RETURNING for INSERT/UPDATE to avoid a separate SELECT query. Check rowCount on DELETE to confirm the row existed.',
        how: 'Parameterized queries use $1, $2, etc. placeholders that are safely escaped by the database driver. The params array is built incrementally as filters are added, and ${params.length} tracks the next parameter index.',
      },
      {
        title: 'Service Layer with Business Logic',
        lines: [79, 94],
        what: 'The UserService contains business rules: getUserById throws if not found, createUser checks for duplicate emails before creating. It depends only on the IUserRepository interface, not on PostgresUserRepository.',
        why: 'Business logic belongs in the service layer, not in the repository or controller. The service orchestrates repository calls and enforces invariants like "no duplicate emails." This keeps repositories simple and focused on data access.',
        when: 'Put business rules in services whenever the logic goes beyond simple CRUD. Validation ("does this email exist?"), authorization ("can this user perform this action?"), and orchestration ("create user then send welcome email") all belong here.',
        how: 'The constructor takes IUserRepository (the interface), not PostgresUserRepository (the implementation). This is dependency inversion — the high-level service module depends on an abstraction, not a low-level detail.',
      },
      {
        title: 'Dependency Injection Composition',
        lines: [96, 99],
        what: 'The composition root wires everything together: creates a database connection, passes it to the repository, and passes the repository to the service. This is the only place that knows about concrete implementations.',
        why: 'Centralizing object creation in a composition root means changing the database only requires changing these 3 lines, not the service or any business logic. In production, you would use a DI container for larger applications.',
        when: 'Always compose your dependencies at the application entry point (composition root). In Express apps, this is typically in your app setup file. In tests, this is where you inject mock repositories.',
        how: 'Each layer receives its dependency through constructor injection. The service never imports or references PostgresUserRepository — it only knows about IUserRepository. This is the Dependency Inversion Principle (the D in SOLID) in action.',
      },
    ],
  },
  {
