    ...
    walkthrough: [
      {
        ...
        lines: [1, 1],
        what: 'Defines a function that takes an array of numbers and a target sum, and returns the indices of two numbers that add up to the target.',
        why: 'The function signature clearly communicates the contract: input is an array and a number, output is an array of two indices. This makes the function reusable and testable.',
        when: 'Use this pattern whenever you need to find a pair of elements in a collection that satisfy a condition. Two Sum is a foundational pattern for many interview problems.',
        how: 'The function accepts two parameters: nums (the array to search) and target (the sum we want to achieve). It will return an array of two indices.',
      },
      {
        title: 'Hash Map Initialization',
        lines: [2, 2],
        what: 'Creates an empty Map to store numbers we have already seen and their indices. The key is the number value, the value is its index in the array.',
        why: 'A Map gives O(1) average lookup time, which is the key insight that makes this solution O(n) instead of the brute-force O(n²) approach with nested loops.',
        when: 'Use a hash map whenever you need fast lookups by value. This "seen so far" pattern applies to many problems: finding duplicates, checking membership, and counting frequencies.',
        how: 'JavaScript\'s Map object stores key-value pairs with O(1) average time for get, set, and has operations. We use it as a lookup table indexed by number values.',
      },
      {
        title: 'Iterate Through Array',
        lines: [3, 3],
        what: 'Starts a single-pass loop through the entire array, visiting each element exactly once.',
        why: 'A single pass through the array gives O(n) time complexity. We check and store in the same loop — no need for a separate pass to build the map first.',
        when: 'Single-pass iteration is ideal when you can make decisions about each element using information accumulated from previously visited elements.',
        how: 'The for loop uses index i to track position. We need the index (not just the value) because the problem asks us to return indices, not the numbers themselves.',
      },
      {
        title: 'Calculate Complement',
        lines: [4, 4],
        what: 'For the current number nums[i], calculates what other number we would need to reach the target. This is simply target minus the current number.',
        why: 'Instead of asking "do any two numbers add up to target?", we reframe the question as "have I already seen the complement of this number?" This transforms a two-variable search into a one-variable lookup.',
        when: 'The complement technique works whenever you are looking for pairs that satisfy an arithmetic relationship (sum, difference, product). It reduces a two-pointer problem to a hash lookup.',
        how: 'If target is 9 and current number is 2, the complement is 7. We then check if 7 has been seen before. This arithmetic is O(1) per element.',
      },
      {
        title: 'Check for Complement Match',
        lines: [5, 7],
        what: 'Checks if the complement already exists in our hash map. If it does, we found our answer — return both indices immediately.',
        why: 'By checking the map before inserting the current number, we ensure we never match a number with itself. The map only contains numbers from previous iterations.',
        when: 'This early return pattern is common in search problems. As soon as we find a valid answer, there is no need to continue iterating.',
        how: 'map.has(complement) checks existence in O(1). If found, map.get(complement) retrieves the stored index of the complement. We return both indices as an array.',
      },
      {
        title: 'Store Current Number',
        lines: [8, 8],
        what: 'If no match was found, stores the current number and its index in the map for future iterations to check against.',
        why: 'Building the map incrementally means each number is available as a potential complement for all subsequent numbers. This is why we check before storing — it naturally prevents self-pairing.',
        when: 'This "build as you go" approach is more elegant than a two-pass solution (first build map, then search). It handles edge cases like duplicate values naturally.',
        how: 'map.set(nums[i], i) stores the number as the key and its index as the value. If a number appears again later, its index gets updated, but that is fine since the earlier index was already checked.',
      },
      {
        title: 'Edge Case: No Solution',
        lines: [10, 10],
        what: 'Returns an empty array if no two numbers add up to the target after checking all elements.',
        why: 'Defensive programming — even though the problem states exactly one solution exists, returning an empty array handles unexpected input gracefully rather than returning undefined.',
        when: 'Always include a fallback return for functions that may not find a result. This prevents runtime errors in calling code and makes the function more robust.',
        how: 'If the loop completes without returning, no complement was found for any number. The empty array signals "no solution found" to the caller.',
      },
    ],
  },
  {
